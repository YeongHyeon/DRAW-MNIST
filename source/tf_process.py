...
def training(sess, neuralnet, saver, dataset, epochs, batch_size, canvas_size, sequence_length, print_step=1):
    ...
    for epoch in range(epochs+1):
        if((epoch % print_step == 0) or (epoch == (epochs))):
            ...
            c_seq_tr, loss_recon_tr, loss_kl_tr = sess.run([neuralnet.c, neuralnet.loss_recon, neuralnet.loss_kl], feed_dict={neuralnet.x:x_tr_static})
            c_seq_te, loss_recon_te, loss_kl_te = sess.run([neuralnet.c, neuralnet.loss_recon, neuralnet.loss_kl], feed_dict={neuralnet.x:x_te_static})
